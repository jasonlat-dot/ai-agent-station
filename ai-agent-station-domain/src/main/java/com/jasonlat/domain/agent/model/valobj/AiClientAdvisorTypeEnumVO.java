...
public enum AiClientAdvisorTypeEnumVO {
    ...
    // 静态Map缓存，用于快速查找
    private static final Map<String, AiClientAdvisorTypeEnumVO> CODE_MAP = new ConcurrentHashMap<>();

    // 静态初始化块，在类加载时初始化Map
    static {
        for (AiClientAdvisorTypeEnumVO value : values()) {
    ...
    public static AiClientAdvisorTypeEnumVO getByCode(String code) {
        ...
        if (!StringUtils.hasLength(code) || !StringUtils.hasLength(code.trim())) {
            throw new RuntimeException("error! advisorType " + code + " not exist!");
        }
        return enumVO;
    }
}
